...
def Create_Menu():
    ...
    for item in rights_dict['departments']:
        excel_menu.add_command(label=dep_dict[int(item)], command=lambda just_dep=int(item), department=int(item), conn=conn: Create_Presence_Excel(Create_Presence_Dict(connection_dict, conn, int(Get_Date_From_Callendar(cal_from)[:4]), int(Get_Date_From_Callendar(cal_from)[5:7]), department=department), int(Get_Date_From_Callendar(cal_from)[:4]), int(Get_Date_From_Callendar(cal_from)[5:7]), smk_dict, action_dict, dep_dict, company_dict, city_dict, agr_dict, pos_dict, just_department=just_dep))
